...
export default function AskAI({ document }: AskAIProps) {
  ...
  return (
    <Accordion type="single" collapsible className="w-full space-y-4">
      ...
      <FeatureContainer title="6. Compare Documents">
        <form onSubmit={(e) => {
          e.preventDefault();
          const formData = new FormData(e.currentTarget);
          const doc1 = formData.get('doc1') as string;
          const doc2 = formData.get('doc2') as string;
          runFlow('compare', () => compareDocuments({ doc1, doc2 }));
        }}>
          <Label htmlFor="doc1">Contract A</Label>
          <Textarea id="doc1" name="doc1" defaultValue={document.clauses.slice(0, 5).map(c => c.text).join('\n')} className="mt-1 mb-2" />
          <Label htmlFor="doc2">Contract B</Label>
          <Textarea id="doc2" name="doc2" defaultValue={document.clauses.slice(5, 10).map(c => c.text).join('\n')} className="mt-1 mb-2" />
